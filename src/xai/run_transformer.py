...
from src.models.train_lstm import (build_model_configs, fit_sequence_scaler,
    ...
    set_seed, transform_sequences)

from src.models.train_transformer import train_fixed_epochs
...
OUTPUT_DIR = PROJECT_ROOT / "artifacts" / "xai" / "transformer"
...
OOS_FILE = DATA_DIR / "transformer_oos_predictions.csv"
METRICS_FILE = DATA_DIR / "transformer_fold_metrics.csv"
...
def main() -> None:
    ...
    for test_year in TEST_YEARS:
        ...
        selected_sec = select_sec_features(train_df,
                                           ...
                                           MIN_SEC_COUNT)
        configs = build_model_configs(selected_sec, model_name="TRANSFORMER",)

        for variant_id, config in enumerate(configs):
            variant = ["A", "B", "C"][variant_id]
            ...
            sequence_df, static_df = run_global_sequence_ig(model=model,
                                                            ...
                                                            device=device,
                                                            xai_name=f'transformer/{variant}/test_{test_year}',
                                                            n_steps=32)
    ...
    for _, example in examples.iterrows():
        ...
        run_local_sequence_ig(model=fold["model"],
                              ...
                              event_row=event_row,
                              device=device,
                              xai_name="transformer/C",
                              n_steps=32)

    logger.info("Zakończono XAI transformera")
